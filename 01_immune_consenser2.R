...
library(cowplot)
library(ggExtra)
# load('resource/HNSC_exprSet_vst.R')
...
immune_data <- rbind(sample_subtypes, ss)
...
#########################################################全免疫细胞分组后无生存意义，所以重新选择cox 0.05
immune <- data[,c(1,2,3,5:32)]
...
genes <- colnames(immune)[4:ncol(immune)]
library(survival)
##批量生存分析
res <- data.frame()
...
anno_colors = list(
  ...
  pathologic_N = c("N0" = "#A5B978", "N1" = "#859346", "N2" = "#656D14", "N3" = "#E1E3E3", "unknown" = "#E3E3E3"),
